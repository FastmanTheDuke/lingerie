@if ($data)
    ...
    @if($data['title'] || $data['text'] || $data['surtitle'])
        <div class="relative group overflow-hidden">
            ...
            <div class="top-0 left-0 px-26 py-14 h-full w-full flex flex-col gap-2 max-xl:px-10 max-xl:py-10">
                <div class="bg-[#F4F2EF] h-full w-full max-w-[410px] mx-auto py-12 px-10 flex flex-col gap-4">
                    <div class="title-type3">{{ $data['surtitle'] }}</div> 
                    <div class="title-type9">{{ $data['title'] }}</div> 
                    <div class="content-type4 mt-[25%]">{!! $data['text'] !!}</div> 
                
                </div>
                @if($data['link']) 
                    <div class="max-w-[410px] w-full mx-auto bg-dark group/btn">
                        <a href="{{ $data['link']['url'] }}" target="{{ $data['link']['target'] }}"  rel="@if($data['link']['target'] == '_blank') noopener noreferrer @endif" class="relative btn-bloc text-[#FAFAFA] shrink-0 px-6 py-4 flex justify-between items-center" data-cursor-type="link-text">
    ...
    @else
        ...
        <div class="relative group overflow-hidden py-14 max-xl:px-6 max-xl:py-10 max-md:p-0">
            <img class="h-full w-full object-cover" src="{{$data['visual']['sizes']['large']}}" alt="">
