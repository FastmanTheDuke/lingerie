...
@section('content')
    ...
    <x-archive-filters maxPage="{{$posts['maxPage']}}" :tags="$tags" title="{{pll__('Toutes nos Modes Lingerie')}}" />

    <div id="ajax-posts-container"
        class="grid grid-cols-4 gap-6 gutter-LR pt-16 max-lg:grid-cols-3 max-md:grid-cols-2 max-sm:grid-cols-1">
        @include('partials.mode.archive-posts', ['posts' => $posts['list']])
    </div>
